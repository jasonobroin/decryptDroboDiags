...
This log is a companion to the Eventlog, but tracks higher frequency activity associated with keeping the diskpack in good health, such as defragmentation, scrubbing, zone temperature, which don't fundamentally affect the organization of the diskpack

## UELog.bin

Binary verson of the User Event log viewable in Dashboard
...
Compress core dump from the main user mode process running BeyondRAID
...
Log file from the main process that runs the NAS engine on Linux and interfaces with the VxWorks cores and with Dashboard.
...
Information about Windows if Dashboard runs on that Operating System

## system.log, systemdetailed.log, ioreg.log
